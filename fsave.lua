----------------------------------------
-- save / load共通 [HENPRI adapted]
-- Based on NUKITASHI Save Fix V1.0
-- Fix applied: get_language() → get_language(true) for HENPRI compatibility
----------------------------------------
sv = {}
----------------------------------------
-- 現在のポインタを保存しておく
...
function sv.save()
	...
	if no < init.save_suspend then
		...
		-- 拡張情報
		local nm = "user_savedata"
		if _G[nm] then _G[nm](no) end
...
function sv.changesavetitle(tbl)
	...
	-- [HENPRI FIX] get_language(true) to respect user language preference
	-- (original NUKITASHI version used get_language() which ignores conf.language)
...
function sv.go_exit()
	if not gameexitflag then
		...
		window_buttonexit()				-- exit処理を複数発生させない
