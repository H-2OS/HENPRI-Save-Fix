----------------------------------------
-- お気に入りボイス [PATCHED: HENPRI Save Fix V1.0]
-- favodata/ への独立バックアップ同期を追加
----------------------------------------
-- お気に入りボイス
function favo_init()
...
--	se_ok()
	...
	-- [PATCHED] saveg.dat が破損していたら favodata からリカバリ
...
function favopage_init()
	...
	local tmsk = isFile(mspx.."bt_save_mask.png") and mspx.."bt_save_mask" or isFile(mspx.."mask.png") and mspx.."mask"		-- thumb mask
	local fabg = isFile(mspx.."bt_save_facebg.png") and mspx.."bt_save_facebg" or isFile(mspx.."mask.png") and mspx.."mask"		-- facebg
...
--		local vv = getBtnInfo('obj'..string.format("%02d", i))
		...
		if t then
			...
			local len = tttl and string.len(tttl) or 0
			if (len > 13) then	-- 13文字以上はトリミングする
				-- tttl = utf_substr_mojisuu(tttl, 13, 20, 13, 2)
				-- tttl = tttl.."……"
			elseif tttl then
				-- tttl = utf_substr_mojisuu(tttl, 13, 20, 13)
			end
			...
			-- -- セーブテキスト > MAX なら [……] を付与して残りをカットする kiyopi
			-- -- tttx = mb_substr(tttx, 1, mx)
			-- if string.len(tttx)/3*2 > mt then	-- 全角は3byteなので2バイト扱いで計算する
			-- 	tttx = mb_substr(tttx, 1, tonumber(mt)-4)
			-- 	tttx = tttx.."……"
			-- else
			...
			-- セーブテキスト > MAX なら [……] を付与して残りをカットする kiyopi
			-- tttx = mb_substr(tttx, 1, mx)
			if string.len(tttx)/3*2 > 52 then	-- 全角は3byteなので2バイト扱いで計算する
				-- tttx = mb_substr(tttx, 1, tonumber(mx)-6)
				-- tttx = utf_substr_mojisuu(tttx, 6, 10, 26, 2)
				-- tttx = tttx.."……"
			else
				-- tttx = mb_substr(tttx, 1, mx)
				-- tttx = utf_substr_mojisuu(tttx, 6, 10, 26)
			end
			...
			if fa and fa[1] == "none" then
				...
				if s then
					...
					lyc2{ id=(thid..".1"), file=(base), x=(ss.x), y=(ss.y), clip=(s), mask=(tmsk)}		-- thumb mask
				end
			elseif fa then
				setMWFaceFile(fa, "favoface", thid..".1")	-- //フェイス設置
				tag{"lyprop", id=(thid), intermediate_render="1", intermediate_render_mask=(face)} -- //マスクトリミング
				-- e:tag{"lyprop", id=(id..'.9'), alpha=255}	-- //フェイス背景表示
				lyc2{ id=(id..'.9'), file=(fabg), clickablethreshold=(cd), mask=(tmsk)} --//ボイス画面フェイス背景
			end

			e:tag{"lyprop", id=(id..'.0'), alpha=0}								-- //セーブスロット土台削除 kiyopi
			e:tag{"lyprop", id=(id..'.2'), alpha=255, clickablethreshold=128}	-- //セーブスロット背景 kiyopi
			e:tag{"lyprop", id=(id..'.19'), alpha=255}							-- //セーブスロットタグ kiyopi
			-- e:tag{"lyprop", id=(id..'.29'), alpha=255}							-- //セーブスロットマスク kiyopi
			...
			lyevent{id=(id..'.0'), click="btn_click", over="btn_over", out="btn_out", name="favo", key=(k), mode="disable"}	-- kiyopi
			lyevent{id=(id..'.2'), click="btn_click", over="btn_over", out="btn_out", name="favo", key=(k), mode="enable"}	-- kiyopi
		...
		-- ない
			...
			e:tag{"lyprop", id=(id..'.0'), alpha=255}		-- //セーブスロット土台 kiyopi
			e:tag{"lyprop", id=(id..'.2'), alpha=0}			-- //セーブスロット背景 kiyopi
			e:tag{"lyprop", id=(id..'.19'), alpha=0}		-- //セーブスロットタグ kiyopi
			-- e:tag{"lyprop", id=(id..'.29'), alpha=0}		-- //セーブスロットマスク kiyopi
			-- 消去
			...
			lyevent{id=(id..'.0'), click="btn_click", over="btn_over", out="btn_out", name="favo", key=(k), mode="enable"}	-- kiyopi
			lyevent{id=(id..'.2'), click="btn_click", over="btn_over", out="btn_out", name="favo", key=(k), mode="disable"}	-- kiyopi
			e:tag{"lyprop", id=(id..'.9'), alpha=0}	-- //フェイス背景非表示
...
function favoclick()
	...
	-- [PATCHED] favodata に同期: 個別ファイル + インデックス
	if init.game_favovoice == "on" then
		favo_savefile(no)
		favo_saveindex()
	end
...
function favodelete()
	...
	local v  = sys.favo
	if v[no] then
		local deleted = sys.favo[no]	-- wasm用に先に退避
		wasm_favodelete(deleted)		-- wasmsync
		sys.favo[no] = nil				-- 先にメモリから削除

		-- [PATCHED] favodata からも削除
		if init.game_favovoice == "on" then
			favo_deletefile(no)
			favo_saveindex()
		end
...
function favomove(m)
	...
	-- [PATCHED] favodata 同期: 影響を受けたスロットを更新
	if init.game_favovoice == "on" then
		if sys.favo[m] then
			favo_savefile(m)
		else
