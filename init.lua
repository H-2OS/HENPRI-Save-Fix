...
--		"extend/staff",		-- スタッフロール
		"extend/updater",	-- アップデータ		updater add 20211012
...
--		"ui/sceneback",		-- シーンバック制御
		...
		"ui/dialog",		-- dialog制御
		"ui/title",			-- タイトル画面
		"ui/lang",			-- 言語選択	//３言語対応 kiyopi add 20240523

--		"ui/flow",			-- フローチャート制御	updater add 20211012
		...
		"extra/bgm",		-- おまけBGM
		"extra/etc",		-- OP / ED 鑑賞
		"extra/scenario",	-- シナリオ選択画面 kiyopi
	}
	local luafilefg = {
	...
	-- -- steam
	-- if init.steam == "on" then
	-- 	e:include("system/extend/steam.lua")
	-- end
...
function system_dataloading()
	...
	if not conf then
		flg.firstboot = true --(AD)：初回起動のフラグ	updater add 20211012
...
--		e:include("system/dmmck.lua")		-- DMM認証	updater add 20211012
...
function system_starting()
	...
	-- [PATCHED] 起動時に favodata へ既存のお気に入りボイスを自動エクスポート
	if init.game_favovoice == "on" then
		pcall(function() favo_exportall() end)
	end
...
function screen_init()
	...
	if game.os == "windows" then
		-- screen size
		-- windows_screeninit()	-- updater add 20211012	これ入れるとconfがしぬ
...
--	game.path.facemask	= image..init.face_path..'/mask'
	...
	game.videomode		= conf.videomode	-- ビデオ再生モード	updater add 20211012
...
----------------------------------------
-- ui path / 多言語切り替え		updater add 20211012
...
function screen_crop(id)
	-- tag{"lyprop", id=(id), intermediate_render="2", clip=(game.clip)}	updater add 20211012
